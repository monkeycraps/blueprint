...
class Blueprint
{
    ...
    {
        $this->reader->addNamespace('In\\Blueprint\\Annotation');
        $this->reader->addNamespace('In\\Blueprint\\Annotation\\Method');

        AnnotationRegistry::registerLoader(function ($class) {
            $path = __DIR__ . '/' . str_replace(['In\\Blueprint\\', '\\'], ['', DIRECTORY_SEPARATOR], $class) . '.php';
    ...
    protected function generateContentsFromResources(Collection $resources, $name)
    {
        ...
        $content = stripslashes(trim($contents));

        $content = str_replace($this->getFormat(), $this->getFormat()
            . $this->line(1)
            . $this->getHost()
            , $content);
    ...
    protected function appendParameters(&$contents, Collection $parameters)
    {
        ...
        $parameters->each(function ($parameter) use (&$contents) {
            ...
            if (isset($parameter->default)) {
                $this->appendSection($contents, sprintf('Default: %s', is_array($parameter->default) ? json_encode($parameter->default) : $parameter->default), 2, 1);
            }
     ...
     *
     * @param string                            $contents
     * @param \In\Blueprint\Annotation\Response $response
     ...
     *
     * @param string                           $contents
     * @param \In\Blueprint\Annotation\Request $request
     * @param \In\Blueprint\Resource           $resource
     *
     * @return void
     */
